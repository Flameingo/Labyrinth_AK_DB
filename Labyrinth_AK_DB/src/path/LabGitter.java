...
public class LabGitter extends Lab
{
  ...
  public LabGitter(String angabe, float x, float y, float x2, float y2, float hoehe)
  {
    switch(angabe)
    {
    case "o": float yy2 = y2 - WandBlock.wdbreite / 2; 			// Damit die Wandbreite nicht die gewaehlten Grenzen ueberragt.
    	add(new Gitter((x + x2) / 2 + 0.13f, yy2, boden, 0, 0, 0, Math.abs(x2 - x), hoehe));
    	break;
    case "u": float yy = y + WandBlock.wdbreite / 2;				// Damit die Wandbreite nicht die
    															// gewaehlten Grenzen ueberragt.
    	add(new Gitter((x + x2) / 2 + 0.13f, yy, boden, 0, 0, 0, Math.abs(x2 - x), hoehe));
    	break;
    case "l": float xx = x + WandBlock.wdbreite / 2; //Damit der Rand der Steine nicht die Grenzen des Feldes ueberragt.
    	add(new Gitter(xx, (y + y2) / 2 + 0.13f, boden, 90, 0, 0, Math.abs(y2 - y), hoehe));
    	break;
    case "r": float xx2 = x2 - WandBlock.wdbreite / 2; //Damit der Wandrand nicht die Grenzen des Feldes ueberragt.
